...
def update_balance(account_number, balance): ### function to update balance
    with open(ACCOUNTS_FILE, 'r') as f:
        lines = f.readlines()

    with open(ACCOUNTS_FILE, 'w') as f:
        for line in lines:
            acc_num, name, pwd, bal = line.strip().split(',')
            if acc_num == account_number:
                f.write(f"{acc_num},{name},{pwd},{balance}\n")  # Update balance
            else:
                f.write(line)  # Write other accounts as they were


def get_fd_balance(account_number): ### function to create fd
    total_fd = 0.0
...
def get_sip_balance(account_number): ### function to create sip
    total_sip = 0.0
...
def deposit(account_number, balance): #function for deposit 
    amount = float(input("Enter amount to deposit: "))
    balance += amount
    log_transaction(account_number, "Deposit", amount)
    print(f"Deposit successful! Current balance: {balance}")
    update_balance(account_number, balance)  # Call update_balance to save changes
    return balance

def withdraw(account_number, balance): ### function for withdraw
...
def forget_password(): ### to reset password
    account_number = input("Enter your account number: ")
...
def open_fixed_deposit(account_number, balance): ### open fd function
...
def close_fixed_deposit(account_number, balance): ### close fd funcion
    # Find the FD to close (get details from the user)
    fd_to_close = float(input("Enter the amount of the FD you want to close: "))

    # Read and update the FD file
    updated_fd_data = []
    fd_found = False
    with open(FD_FILE, 'r') as f:
        for line in f:
            acc_num, amount, duration, interest, date = line.strip().split(',')
            if acc_num == account_number and float(amount) == fd_to_close:
                fd_found = True
                # Calculate interest earned (simplified for now)
                interest_earned = float(interest)  # Assuming interest is already calculated
                balance += float(amount) + interest_earned
                print(f"Fixed Deposit of {amount} RUS closed successfully.")
                print(f"Interest earned: {interest_earned} RUS")
                print(f"Updated balance: {balance}")
            else:
                updated_fd_data.append(line)

    if not fd_found:
        print("Fixed Deposit not found.")
        return balance

    # Write the updated FD data back to the file
    with open(FD_FILE, 'w') as f:
        f.writelines(updated_fd_data)

    return balance


def open_sip(account_number, balance): ### open sip function
...
def show_balance(account_number): ### show balnce function
